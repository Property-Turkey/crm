...
class PropertiesController extends AppController
{
    ...
    public function index($_pid = null)
    {
        ...
        if ($this->request->is('post')) {
            ...
            // if (isset($_pid)) {
            //     $conditions['Properties.source_id'] = $_pid;
            // }
            if (strlen($_k . '') > 0) {
                if ($_method == 'like') {
            ...
            if ($_k !== false) {
                $_method == 'like' ? $conditions[$_col . ' LIKE '] = '%' . $_k . '%' : $conditions['Properties.' . $_col] = $_k; // note = $col condition is not correct 
            }
            ...
            // LIST
            if (!empty($_list)) {
                $data = $this->paginate($this->Properties, [
                    "order" => [$_col => $_dir],
    ...
    public function delete($id = null)
    {
        ...
        $delRec = $this->Properties->deleteAll(['id IN ' => explode(",", $id)]);
    ...
    public function enable($ids = null)
    {
        ...
        foreach ($records as $rec) {
            ...
            $dt = $this->Properties->newEmptyEntity();
            ;
            $dt["id"] = $rec;
            if (!$this->Properties->save($dt)) {
                $errors[] = $dt->getErrors();
            }
        }
