    ...
    private function notifications()
    {
        if ($this->authUser["id"]) {
            ...
            if (in_array($this->authUser['user_role'], ['admin.supervisor'])) {
                ...
                $office_members_ids = $this->getTableLocator()->get('Users')->find('list', [
                    'conditions' => [
            ...
            if (!$user) {
                ...
                if ($this->Do->adder($updt, 'Users')) {
                    ...
                    if (!empty($dt['changePassword'])) {
                        echo json_encode(['status' => 'SUCCESS', 'data' => $user, 'redirect' => $this->app_folder . '/admin/myaccount?msg=' . __('change_password')]);
                        die();
                    }

                    // tell user activition succeed
                    if ($user['rec_state'] == 0 && isset($dt['autologin'])) {
                        echo json_encode(['status' => 'SUCCESS', 'data' => $user, 'redirect' => $this->app_folder . '/?activate=1']);
                        die();
                    }
                    echo json_encode(['status' => 'SUCCESS', 'data' => $user]);
                    die();
                } else {
                    echo json_encode(['status' => 'FAIL', 'data' => $user, 'updt' => $updt]);
                    die();
                }
            }
    ...
    public function getpassword($code = null)
    {
        ...
        if ($this->request->is('post')) {
            ...
            if (!filter_var($dt['email'], FILTER_VALIDATE_EMAIL)) {
                echo json_encode(['status' => 'FAIL', 'msg' => __('is-email-msg'), 'data' => []]);
                die();
            }
