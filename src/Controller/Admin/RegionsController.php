...
class RegionsController extends AppController
{
    ...
    {   
        $_region = isset($_GET['tags']) ? $_GET['tags'] : false;
        $_keyword = isset($_GET['keyword']) ? $_GET['keyword'] : false;

        // TAGS search result for tags
        if (!empty($_region)) {
            $regionCondition = [
                'region_name LIKE' => '%' . $_keyword . '%'
            ];

            $data = $this->Regions
                ->find('all')
                ->select(['text' => 'region_name', 'value'=>'id'])
        ...
        if ($this->request->is('post')) {
            ...
            // if (isset($_pid)) {
            //     $conditions['Regions.source_id'] = $_pid;
            // }
            if (strlen($_k.'') > 0) {
                if ($_method == 'like') {
            ...
            if ($_k !== false) {
                $_method == 'like' ? $conditions[$_col . ' LIKE '] = '%' . $_k . '%' : $conditions['Regions.' . $_col] = $_k; // note = $col condition is not correct 
            }
            ...
            echo json_encode(
                [
                    ...
                    "paging" => $this->Paginator->getPagingParams()["Regions"]
                ],
    ...
    public function save($id = -1)
    {
        ...
        if ($this->request->is(['patch', 'put'])) {
            ...
            $rec = $this->Regions->get($dt['id']);
            $rec = $this->Regions->patchEntity($rec, $dt); 
        ...
        if ($this->request->is(['post', 'patch', 'put'])) {
            ...
            unset($rec['user']);
            unset($rec['lead']);
            if ($newRec = $this->Regions->save($rec)) {
    ...
    public function delete($id = null)
    {
        ...
        $delRec = $this->Regions->deleteAll(['id IN ' => explode(",", $id)]);
    ...
    public function enable($val=1, $ids=null)
    {
        ...
        foreach($records as $rec){
            ...
            $dt= $this->Regions->newEmptyEntity();;
            $dt["id"] = $rec;
            $dt["rec_state"] = $val;
            if(!$this->Regions->save($dt)){
                $errors[] = $dt->getErrors();
            }
        }
